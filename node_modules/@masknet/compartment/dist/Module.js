...
export class Module {
            ...
            if (resolution !== null) {
                if (starResolution === null)
                    ...
                    if ((resolution.bindingName === namespace && starResolution.bindingName !== namespace) ||
                        ...
                        return ambiguous;
                    if (typeof resolution.bindingName === 'string' &&
    ...
    static #InnerModuleLinking(module, stack, index) {
        if ([ModuleStatus.linking, ModuleStatus.linked, ModuleStatus.evaluatingAsync, ModuleStatus.evaluated].includes(module.#Status)) {
            return index;
        }
        assert(module.#Status === ModuleStatus.unlinked);
    ...
    static #GatherAvailableAncestors(module, execList) {
        for (const m of module.#AsyncParentModules) {
            if (!execList.includes(m) && m.#CycleRoot.#EvaluationError === empty) {
                ...
                if (m.#PendingAsyncDependencies === 0) {
                    execList.push(m);
                    if (!m.#HasTLA)
                        this.#GatherAvailableAncestors(m, execList);
                }
            }
        }
    }
...
(function (ModuleStatus) {
    ModuleStatus[ModuleStatus["unlinked"] = 0] = "unlinked";
    ModuleStatus[ModuleStatus["linking"] = 1] = "linking";
    ModuleStatus[ModuleStatus["linked"] = 2] = "linked";
    ModuleStatus[ModuleStatus["evaluating"] = 3] = "evaluating";
    ModuleStatus[ModuleStatus["evaluatingAsync"] = 4] = "evaluatingAsync";
    ModuleStatus[ModuleStatus["evaluated"] = 5] = "evaluated";
...
const moduleNamespaceExoticMethods = {
    ...
    defineProperty(target, p, attributes) {
        ...
        if (attributes.configurable)
            return false;
        if (attributes.enumerable === false)
            return false;
        if (attributes.get || attributes.set)
            return false;
        if (attributes.writable === false)
            return false;
        if ('value' in attributes)
            return Object.is(current.value, attributes.value);
        return true;
    },
    set() {
        return false;
