...
const ArrayUnscopables = Object.freeze(Object.assign({
  __proto__: null
...
const proxyTargetToLazyPropertyDescriptorStateMap = toSafeWeakMap(new WeakMap()); // istanbul ignore next
...
function createMembraneMarshall(globalObject) {
  ...
  const ArrayBufferProtoByteLengthGetter = !IS_IN_SHADOW_REALM ? ReflectApply(ObjectProtoLookupGetter, ArrayBufferCtor.prototype, ['byteLength']) : undefined;
  ...
  const TypedArrayProtoLengthGetter = ReflectApply(ObjectProtoLookupGetter, // eslint-disable-next-line no-proto
  ...
  return function createHooksCallback(color, foreignCallableHooksCallback, options) {
    ...
    function getTransferablePointer(originalTarget, foreignCallablePusher = foreignCallablePushTarget) {
      ...
      proxyPointer = foreignCallablePusher(createPointer(distortedTarget), targetTraits, targetFunctionArity, targetFunctionName, targetTypedArrayLength); // The WeakMap is populated with the original target rather then the
    ...
    BoundaryProxyHandler.hybridGetTrap = IS_IN_SHADOW_REALM ? function (_shadowTarget, key, receiver) {
      ...
      } else if (key === SymbolToStringTag && foreignTargetTraits & 16
      /* IsObject */
      ) {
        let toStringTag;
    ...
    BoundaryProxyHandler.hybridGetTrapForTypedArray = IS_IN_SHADOW_REALM ? function (_shadowTarget, key, receiver) {
      ...
      if (possibleIndex > -1 && possibleIndex < foreignTargetTypedArrayLength && NumberIsInteger(possibleIndex)) {
        try {
          result = foreignCallableGetTypedArrayIndexedValue(foreignTargetPointer, key);
        } catch (error) {
    ...
    BoundaryProxyHandler.defaultSetPrototypeOfTrap = IS_IN_SHADOW_REALM ? BoundaryProxyHandler.pendingSetPrototypeOfTrap : BoundaryProxyHandler.passthruSetPrototypeOfTrap; // Export callable hooks to a foreign realm.
    ...
    } : noop, // callableInstallErrorPrepareStackTrace
    ...
    !IS_IN_SHADOW_REALM ? targetPointer => {
      ...
      if (target === null || target === undefined || target === ObjectProto || target === RegExpProto) {
        return false;
      }
      ...
      if (typeof target === 'object') {
        ...
        try {
          if (ReflectGetPrototypeOf(target) === null && ( // Ensure `value` is not an `Object.prototype` from an iframe.
...
class VirtualEnvironment {
  constructor(options) {
    ...
    blueConnect(undefined, // redGlobalThisPointer,
    ...
    redCallablePushErrorTarget, redCallablePushTarget, redCallableApply, redCallableConstruct, redCallableDefineProperty, redCallableDeleteProperty, redCallableGet, redCallableGetOwnPropertyDescriptor, redCallableGetPrototypeOf, redCallableHas, redCallableIsExtensible, redCallableOwnKeys, redCallablePreventExtensions, redCallableSet, redCallableSetPrototypeOf, redCallableDebugInfo, undefined, // redCallableDefineProperties,
    redCallableGetLazyPropertyDescriptorStateByTarget, redCallableGetTargetIntegrityTraits, redCallableGetToStringTagOfTarget, redCallableGetTypedArrayIndexedValue, redCallableInstallErrorPrepareStackTrace, undefined, // redCallableInstallLazyPropertyDescriptors,
    redCallableIsTargetLive, redCallableIsTargetRevoked, redCallableSerializeTarget, redCallableSetLazyPropertyDescriptorStateByTarget, redCallableBatchGetPrototypeOfAndGetOwnPropertyDescriptors, redCallableBatchGetPrototypeOfWhenHasNoOwnProperty, redCallableBatchGetPrototypeOfWhenHasNoOwnPropertyDescriptor);
    ...
    blueCallablePushErrorTarget, blueCallablePushTarget, blueCallableApply, blueCallableConstruct, blueCallableDefineProperty, blueCallableDeleteProperty, blueCallableGet, blueCallableGetOwnPropertyDescriptor, blueCallableGetPrototypeOf, blueCallableHas, blueCallableIsExtensible, blueCallableOwnKeys, blueCallablePreventExtensions, blueCallableSet, blueCallableSetPrototypeOf, blueCallableDebugInfo, undefined, // blueCallableDefineProperties,
    blueCallableGetLazyPropertyDescriptorStateByTarget, blueCallableGetTargetIntegrityTraits, blueCallableGetToStringTagOfTarget, blueCallableGetTypedArrayIndexedValue, blueCallableInstallErrorPrepareStackTrace, undefined, // blueCallableInstallLazyPropertyDescriptors,
    blueCallableIsTargetLive, blueCallableIsTargetRevoked, blueCallableSerializeTarget, blueCallableSetLazyPropertyDescriptorStateByTarget, blueCallableBatchGetPrototypeOfAndGetOwnPropertyDescriptors, blueCallableBatchGetPrototypeOfWhenHasNoOwnProperty, blueCallableBatchGetPrototypeOfWhenHasNoOwnPropertyDescriptor);
  ...
  remapProperties(target, unsafeBlueDescMap) {
    if (typeof target === 'object' && target !== null || typeof target === 'function') {
      ...
      for (let i = 0, j = 1; i < length; i += 1, j += 7) {
        ...
        const safeBlueDesc = ObjectAssign$1({
          __proto__: null
        }, unsafeBlueDesc);
        args[j] = ownKey;
...
export { VirtualEnvironment, assignFilteredGlobalDescriptorsFromPropertyDescriptorMap, createBlueConnector, createMembraneMarshall, createRedConnector, getFilteredGlobalOwnKeys, linkIntrinsics, toSafeArray, toSafeWeakMap };
